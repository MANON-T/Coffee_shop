...
if ($row > 0) {
    ...
    while ($rdm = mysqli_fetch_assoc($rdm_query)) {
        ...
        if ($water_row > 0) {
            ...
            VALUES ('{$last_id}','{$water_ID}', '{$water_name}', '{$water_option}', '0', '1', '0')") or die('query failed');
        ...
        if ($dess_row > 0) {
            ...
            $query = mysqli_query($conn, "INSERT INTO order_detail(ord_orderID, ord_productID, ord_productName, ord_option, ord_price, ord_quantity, ord_totalPrice) 
            VALUES ('{$last_id}','{$dessert_ID}', '{$dessert_name}', '-', '0', '1', '0')") or die('query failed');
            $query1 = mysqli_query($conn, "UPDATE redeem SET rd_status = 'redeemed' WHERE rd_redeemID = '{$ID}'");
        }
        ...
        if ($fruit_row > 0) {
            ...
            $query = mysqli_query($conn, "INSERT INTO order_detail(ord_orderID, ord_productID, ord_productName, ord_option, ord_price, ord_quantity, ord_totalPrice) 
            VALUES ('{$last_id}','{$fruit_ID}', '{$fruit_name}', '-', '0', '1', '0')") or die('query failed');
            $query1 = mysqli_query($conn, "UPDATE redeem SET rd_status = 'redeemed' WHERE rd_redeemID = '{$ID}'");
        }
    }
